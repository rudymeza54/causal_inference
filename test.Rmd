...
knitr::include_graphics( "Picture1.1.htm" )
```
...
knitr::include_graphics( "Picture3.1.htm" )
```
...
knitr::include_graphics( "Housing.htm" )
```
...
House_Price <- round(House_Price,0)
#House_Price = log(Y)
data <- as.data.frame(cbind(House_Price, Group, Post_Treatment))
# write.csv( data, "data/diff-in-diff-housing-example.csv", row.names=F )
```
...
1. The coefficient $\beta_0$ (the constant) is significant and different from zero. Houses in the control group have an average price of \$216,672 in time = 1.
...
4. Finally, the coefficient $\beta_3$ (Diff-in-Diff) is significant and different from zero, which means that the construction of an affordable housing site change the price of nearby houses. Particularly, the price increases of \$34,066 more than it would have without the construction of the housing site. 
...
knitr::include_graphics( "Minimum Wage.htm" )
```
